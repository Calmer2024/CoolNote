...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let saves = SaveService::new(library_id.into(), database.clone(), RecoveryStore::new(root.path().join("recovery"))?);

    let large_text = "编".repeat(LARGE_TEXT_CHARS);
    let text_note = workspace.create_note(None, "十万字", None)?;
    let text_document = json!({"schemaVersion":1,"type":"doc","content":[paragraph(0,&large_text)]});
    let text_markdown = format!("# 十万字\n\n{large_text}\n");
    let started = Instant::now();
    saves.save_note(SaveNoteRequest { note_id:text_note.id.clone(),base_revision:text_note.revision,client_transaction_id:Uuid::new_v4().to_string(),title:"十万字".into(),document_json:text_document,markdown_snapshot:text_markdown.clone() })?;
    ...
    let content=(0..LARGE_BLOCK_COUNT).map(|index|paragraph(index,"可编辑内容块")).collect::<Vec<_>>();
    let block_document=json!({"schemaVersion":1,"type":"doc","content":content});
    let block_markdown=format!("# 五千块\n\n{}\n",(0..LARGE_BLOCK_COUNT).map(|_|"可编辑内容块").collect::<Vec<_>>().join("\n\n"));
    let started=Instant::now();
    saves.save_note(SaveNoteRequest { note_id:block_note.id.clone(),base_revision:block_note.revision,client_transaction_id:Uuid::new_v4().to_string(),title:"五千块".into(),document_json:block_document,markdown_snapshot:block_markdown.clone() })?;
    ...
    database.with_write(|tx| { tx.execute("UPDATE notes SET document_json='corrupted' WHERE id=?1",[&block_note.id])?; Ok(()) })?;
...
fn report(name:&str,started:Instant,threshold_ms:f64)->Result<(),Box<dyn std::error::Error>> {
    let elapsed=started.elapsed().as_secs_f64()*1000.0;
    println!("metric={name} elapsed_ms={elapsed:.2} threshold_ms={threshold_ms:.2}");
    if elapsed>threshold_ms { return Err(format!("{name} {elapsed:.2}ms exceeds {threshold_ms:.2}ms").into()) }
    Ok(())
}
