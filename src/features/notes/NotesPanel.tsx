...
type Props = {
  ...
  title: string; sortDirection: 'asc' | 'desc'; view: NoteView; categories: CategoryDto[]
  ...
  onCreate: () => void; onImport: () => void; showCreate: boolean; onEmptyTrash: () => void
}
  ...
  return <section ref={panelRef} className="notes-panel" aria-label="笔记列表" data-collapsed={props.collapsed} data-selecting={selectedIds.size > 0}>
    <div className="notes-header">
      {selectedIds.size ? <div className="selection-bar"><strong>已选择 {selectedIds.size} 项</strong>
        ...
        {props.view!=='trash'&&<button title="置顶" aria-label="置顶" onClick={() => props.onAction(selected, 'pin')}><Icon name="pin" /></button>}
      ...
      </div> : <><div className="notes-heading"><strong>{props.title}</strong><span className="notes-count">{total}</span></div><div className="notes-header-actions">{props.view==='trash'&&<button className="icon-button empty-trash-button" aria-label="清空回收站" title="一键清空回收站" onClick={props.onEmptyTrash}><Icon name="trash-2"/></button>}{props.showCreate&&<button ref={createButtonRef} className="icon-button note-create-menu-trigger" aria-label="新建或导入笔记" aria-expanded={createMenu} onClick={()=>setCreateMenu(value=>!value)}><Icon name="plus"/></button>}<button className="icon-button sort-button" data-direction={props.sortDirection} aria-label={`更新时间${props.sortDirection === 'desc' ? '降序' : '升序'}`} title="切换更新时间排序" onClick={() => props.onDirection(props.sortDirection === 'desc' ? 'asc' : 'desc')}><Icon name={props.sortDirection === 'desc' ? 'sort-desc' : 'sort-asc'} /></button></div></>}
    </div>
    ...
    {props.loading ? <div className="notes-list notes-empty">正在打开笔记库…</div> : notes.length ?
      <div className="notes-list" data-sort-direction={props.sortDirection} role="listbox" tabIndex={0} onKeyDown={keydown}>
        {notes.map((note, index) => <div className={`note-card-row prototype-note-row${note.id === selectedNoteId ? ' selected' : ''}`} key={note.id} onContextMenu={(event) => { event.preventDefault(); setMenu({note,x:event.clientX,y:event.clientY,mode:'note'}) }}>
          <input type="checkbox" aria-label={`选择${noteTitle(note.title)}`} checked={selectedIds.has(note.id)} onChange={() => props.onToggleSelected(note.id)} />
          <button className="note-card" type="button" aria-current={index === activeIndex ? 'true' : undefined} onClick={() => { setActiveIndex(index); props.onSelect(note.id) }}>
            ...
            <div className="note-meta"><time>{updatedLabel(note.updatedAt)}</time>{props.categories.find(category=>category.id===note.categoryId)&&<span className="note-category-token">{props.categories.find(category=>category.id===note.categoryId)!.name}</span>}{note.tags.slice(0,2).map((tag) => <span className="tag" key={tag.id}>{tag.name}</span>)}<span className="note-flags">{note.isPinned && <Icon name="pin" />}{note.isFavorite && <Icon name="star" />}</span></div>
          </button>
          <div className="note-hover-actions">{props.view!=='trash'&&<button className={`icon-button${note.isFavorite?' is-active':''}`} aria-label={note.isFavorite?'取消收藏':'收藏'} onClick={() => act(note,note.isFavorite?'unfavorite':'favorite')}><Icon name="star" /></button>}<button className="icon-button" aria-label="更多" onClick={(event) => { event.stopPropagation(); const rect=event.currentTarget.getBoundingClientRect(); setMenu({note,x:rect.left,y:rect.bottom+4,mode:'note'}) }}><Icon name="ellipsis" /></button></div>
        </div>)}
    ...
    <FloatingLayer open={!!menu} point={menu?{left:menu.x,top:menu.y}:undefined} className="product-menu note-list-menu" role="menu" onDismiss={()=>{setMenu(null);setDeleteConfirm(null)}}>
      ...
      {menu.mode === 'move' ? <><div className="menu-label">移动到分类</div>{props.categories.map(category=><button key={category.id} onClick={()=>{props.onMove(selectedIds.size ? selected : [menu.note.id],category.id);setMenu(null)}}><Icon name={category.iconName || 'folder'} /><span>{category.name}</span></button>)}</> : <>
        {props.view!=='trash'&&<><button onClick={()=>act(menu.note,menu.note.isPinned?'unpin':'pin')}><Icon name="pin" /><span>{menu.note.isPinned?'取消置顶':'置顶'}</span></button><button onClick={()=>act(menu.note,menu.note.isFavorite?'unfavorite':'favorite')}><Icon name="star" /><span>{menu.note.isFavorite?'取消收藏':'收藏'}</span></button><button onClick={()=>setMenu({...menu,mode:'move'})}><Icon name="folder" /><span>移动到</span><Icon name="chevron-right" /></button><div className="separator" /></>}
